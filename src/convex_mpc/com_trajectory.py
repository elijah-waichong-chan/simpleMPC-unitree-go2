...
class ComTraj:
    ...
    def generate_traj(
        ...
        self.pos_traj_world[:, :] = self.pos_des_world.reshape(3, 1) + (
            vel_desired_world.reshape(3, 1) * t_vec.reshape(1, N)
        )
        ...
        [r_fl_next_td_world, r_fr_next_td_world, r_rl_next_td_world, r_rr_next_td_world] = (
    ...
    def _skew(self, vector):
        ...
        return np.array(
            [[0, -vector[2], vector[1]], [vector[2], 0, -vector[0]], [-vector[1], vector[0], 0]]
        )
    ...
    def _continuousDynamics(self, go2):
        ...
        R_z = np.array(
            [[cos(yaw_avg), -sin(yaw_avg), 0], [sin(yaw_avg), cos(yaw_avg), 0], [0, 0, 1]]
        )
    ...
    def _discreteDynamics(self, dt: float):
        ...
        RzT = np.array([[cy, sy, 0.0], [-sy, cy, 0.0], [0.0, 0.0, 1.0]], dtype=float)
