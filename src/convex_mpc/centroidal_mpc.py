...
class CentroidalMPC:
    ...
    def solve_QP(self, go2: PinGo2Model, traj: ComTraj, verbose: bool = False):
        ...
        if verbose:
            ...
            print(
                f"[QP SOLVER] total time = {(t_compute + t_solve)*1e3:.3f} ms  ({1.0/(t_compute + t_solve):.1f} Hz)"
            )
    ...
    def _build_sparse_matrix(self, traj: ComTraj, verbose: bool = False):
        ...
        if verbose:
            ...
            print(
                f"  H: {nH_rows:4d} x {nH_cols:<4d} | nnz = {nH_nnz:6d} | dens = {nH_nnz / nH_tot:7.4f}"
            )
            print(
                f"  A: {nA_rows:4d} x {nA_cols:<4d} | nnz = {nA_nnz:6d} | dens = {nA_nnz / nA_tot:7.4f}"
            )
