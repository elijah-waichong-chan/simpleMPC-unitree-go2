"""
Demo 06: PID-DE-MPC for quadruped locomotion.

Wraps the standard centroidal MPC with PID-based event-triggering and an
Extended State Observer (ESO), reducing the number of QP solves while
maintaining tracking performance.

Command schedule (simpler than ex00 for stability):
  0-2.0 s   : walk forward at 0.5 m/s
  2.0-3.0 s : stop
  3.0-5.0 s : walk forward at 0.5 m/s
  5.0-6.0 s : stop

Compare with ex00_demo.py: the CentroidalMPC is replaced by CentroidalPIDDEMPC
which only re-solves the QP when the PID event-trigger fires.
"""
...
CMD_SCHEDULE = [
    BodyCmdPhase(0.0, 2.0, 0.5, 0.0, 0.27, 0.0),
    BodyCmdPhase(2.0, 3.0, 0.0, 0.0, 0.27, 0.0),
    BodyCmdPhase(3.0, 5.0, 0.5, 0.0, 0.27, 0.0),
    BodyCmdPhase(5.0, 6.0, 0.0, 0.0, 0.27, 0.0),
]
...
# Use CentroidalPIDDEMPC instead of CentroidalMPC
...
for k in range(SIM_STEPS):
    ...
    if (k % CTRL_DECIM) == 0 and ctrl_i < CTRL_STEPS:
        ...
        # MPC update (wrapped with PID event-trigger + ESO)
        ...
        # Compute joint torques (same as ex00_demo)
...
plot_swing_foot_traj(t_vec, foot_traj, False, label=LABEL)
plot_mpc_result(t_vec, mpc_force_world, tau_cmd, x_vec, block=False, label=LABEL)

# MPC solve time plot (highlight reuses as zero solve time)
fig, axes = plt.subplots(2, 1, figsize=(10, 6))
mpc_steps = np.arange(len(mpc_solve_time_ms)) * MPC_DT
axes[0].bar(mpc_steps, mpc_solve_time_ms, width=MPC_DT * 0.8, color="steelblue")
axes[0].set_ylabel("Solve time [ms]")
axes[0].set_title("MPC Solve Time (PID-DE-MPC)")
axes[0].grid(True, alpha=0.3)

axes[1].bar(mpc_steps, mpc_update_time_ms, width=MPC_DT * 0.8, color="darkorange")
axes[1].set_xlabel("Time [s]")
axes[1].set_ylabel("Update time [ms]")
axes[1].set_title("MPC Update Time (incl. event-trigger check)")
axes[1].grid(True, alpha=0.3)
plt.tight_layout()

plt.savefig("./assets/ex06_pid_de_mpc_solve_time.png", dpi=300)
